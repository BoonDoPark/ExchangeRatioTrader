...
from PyQt5.QtWidgets import QMainWindow, QTableWidget, QTableWidgetItem, QSpinBox
...
class TabVisualize:
    ...
    def refresh_table_widget(self):
        ...
        duration = self.window.spinBox_duration_2.value()
    ...
    def on_clicked_refresh_chart(self, item: QTableWidgetItem):
        ...
        for key_date in key_dates:
            ...
            exchange_ratios = self.proc_receive.ratios.get(key_date)
            filtered_exchange_ratios = list(
                filter(lambda exchange_ratio: exchange_ratio.cur_unit == selected_cur_unit, exchange_ratios))
